...
16. [Guide d'intégration LemonSqueezy](#16-guide-dintégration-lemonsqueezy)
...
**Response 422 (événement passé) :**

```json
{
  "message": "L'événement est déjà passé, le remboursement n'est plus possible."
}
```
...
Événements terminés (status = DONE), triés par date décroissante (plus récents en premier).
...
[
  {
    "id": 1,
    "title": "Difract Night - Janvier",
    "location": "Paris",
    "event_date": "2026-01-15T22:00:00.000000Z",
    "status": "DONE",
    "image_url": "https://apidifract.kazllrd.fr/storage/media/events/1/cover/uuid.jpg",
    "artists": [
      /* tableau d'artistes ayant participé */
    ],
    "votes": [
      /* tous les votes de l'événement */
    ],
    "styles": [{ "id": 1, "name": "House" }],
...
> Les champs `artists` et `votes` sont **déjà chargés**, évitant ainsi des appels API supplémentaires.
...
### Guide d'intégration frontend — Achat de billets

> Ce flux est similaire à celui des abonnements : le backend génère une `checkout_url` LemonSqueezy, le front l'ouvre dans un navigateur / WebView, et le webhook confirme le paiement automatiquement.

#### Flux complet d'achat

```
┌──────────────────────────────────────────────────────────────────┐
│  1. Vérifier le prix et la disponibilité                        │
│     GET /events/{eventId}/ticket-price                          │
│     → sale_available, tier, price, tier_available               │
│                                                                  │
│  2. Proposer l'option assurance (+2,99€)                        │
│     Afficher un toggle sur l'écran d'achat                      │
│                                                                  │
│  3. Générer le lien de checkout                                 │
│     POST /events/{eventId}/tickets/checkout                     │
│     Body : { "has_insurance": true/false }                      │
│     → checkout_url                                              │
│                                                                  │
│  4. Ouvrir la checkout_url                                      │
│     • Mobile : WebView (react-native-webview) ou Linking.openURL│
│     • Web : window.open() ou redirection                        │
│                                                                  │
│  5. L'utilisateur paie sur LemonSqueezy                         │
│     (page de paiement hébergée par LS, pas de code frontend)    │
│                                                                  │
│  6. Webhook automatique (côté serveur)                          │
│     LS envoie un event "order_created" au backend               │
│     → Le billet est créé avec status = "confirmed"              │
│     → Un qr_code UUID unique est généré                         │
│                                                                  │
│  7. Récupérer le billet confirmé                                │
│     GET /me/tickets                                             │
│     → polling ou navigation vers "Mes billets"                  │
└──────────────────────────────────────────────────────────────────┘
```

#### Implémentation recommandée (React Native)

**Étape 1 — Écran événement : vérifier la disponibilité**

```javascript
const response = await fetch(`${API_BASE}/events/${eventId}/ticket-price`, {
  headers: { Authorization: `Bearer ${token}` },
});
const data = await response.json();

if (!data.sale_available) {
  // Afficher un message selon data.reason :
  // "voting_open"  → "La vente ouvrira le {data.sale_opens_at}"
  // "sale_closed"  → "La vente est fermée pour le moment"
  return;
}

// Afficher : tier, price, tier_available
```

**Étape 2 — Lancer le checkout**

```javascript
const checkoutResponse = await fetch(
  `${API_BASE}/events/${eventId}/tickets/checkout`,
  {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ has_insurance: userWantsInsurance }),
  },
);
const checkout = await checkoutResponse.json();

// Ouvrir l'URL de checkout LemonSqueezy (identique aux abonnements)
Linking.openURL(checkout.checkout_url);
// OU en WebView :
// navigation.navigate('Checkout', { url: checkout.checkout_url });
```

**Étape 3 — Après paiement : récupérer le billet**

```javascript
// Le webhook crée le billet côté serveur (~2-10 secondes après paiement).
// Deux stratégies possibles :

// Option A — Polling (recommandé, simple)
const pollForTicket = async (eventId, maxRetries = 10) => {
  for (let i = 0; i < maxRetries; i++) {
    ...
    if (ticket && ticket.status === "confirmed") {
      return ticket; // ✅ Billet confirmé
    }
  }
  return null; // Timeout — proposer de rafraîchir manuellement
};

// Option B — Navigation directe vers "Mes billets"
// Rediriger vers l'écran /me/tickets après fermeture du WebView
```

#### Flux de remboursement (assurance)

```javascript
// Uniquement si has_insurance = true ET avant la date de l'événement
const refundResponse = await fetch(`${API_BASE}/me/tickets/${ticketId}`, {
  method: "DELETE",
  headers: { Authorization: `Bearer ${token}` },
});

// 200 → remboursement effectué (montant total : prix billet + 2,99€ assurance)
// 403 → pas d'assurance sur ce billet
// 422 → événement déjà passé
```

#### Gestion des erreurs de checkout

| Code retour               | Cause                          | Action frontend                     |
| ------------------------- | ------------------------------ | ----------------------------------- |
| 200                       | Checkout généré                | Ouvrir `checkout_url`               |
| 409                       | Billet déjà acheté             | Afficher "Vous avez déjà un billet" |
| 422 (vente indisponible)  | Hors fenêtre de vente          | Afficher "Vente fermée"             |
| 422 (stock épuisé global) | Plus de places                 | Afficher "Complet"                  |
| 422 (stock palier épuisé) | Plus de billets pour ce palier | Afficher "Plus de billets {tier}"   |
| 500                       | Variant LS non configuré       | Afficher erreur technique           |

#### QR Code — Affichage et scan

Le billet confirmé contient un champ `qr_code` (UUID). Le frontend doit :

1. **Afficher le QR code** dans l'écran "Mes billets" (utiliser une lib comme `react-native-qrcode-svg` ou `qrcode.react`)
2. **Scanner le QR code** à l'entrée de l'événement (organisateurs uniquement) via `POST /tickets/{qrCode}/scan`
...
// Scanner à l'entrée (côté organisateur)
const scanResult = await fetch(`${API_BASE}/tickets/${scannedQrCode}/scan`, {
  method: "POST",
  headers: { Authorization: `Bearer ${token}` },
});
const result = await scanResult.json();
// result.valid = true  → Entrée autorisée
// result.valid = false → Afficher result.reason
```
...
## 16. Guide d'intégration LemonSqueezy

> Ce guide résume les principes communs à **tous les paiements** (abonnements + billets) via LemonSqueezy.

### Principe général

LemonSqueezy fonctionne en **mode checkout hébergé** : le backend génère une URL, le frontend l'ouvre, et le paiement est traité entièrement côté LS. Les webhooks confirment le résultat.

```
Frontend                    Backend                     LemonSqueezy
   │                           │                              │
   ├── POST /checkout ────────►│                              │
   │                           ├── Crée checkout URL ────────►│
   │◄── { checkout_url } ──────┤                              │
   ...
   │   (WebView / navigateur)  │                              │
   │                           │                              │
   │              Utilisateur paie sur la page LS              │
   │                           │                              │
   │                           │◄── Webhook (order_created) ──┤
   │                           │    Crée ticket / subscription │
   │                           │                              │
   ├── GET /me/tickets ───────►│                              │
   │◄── [ticket confirmé] ─────┤                              │
```

### Ouverture du checkout (mobile)

```javascript
import { Linking } from "react-native";

// Méthode simple — ouvre le navigateur système
Linking.openURL(checkout_url);

// OU avec react-native-webview pour garder l'utilisateur dans l'app
import { WebView } from "react-native-webview";

<WebView
  source={{ uri: checkout_url }}
  onNavigationStateChange={(navState) => {
    // Détecter la page de confirmation LS pour fermer la WebView
    if (navState.url.includes("receipt")) {
      // Fermer la WebView et lancer le polling
      navigation.goBack();
      pollForConfirmation();
    }
  }}
...
### Résumé des routes par flux

| Flux           | Routes                                            | Méthode                 |
| -------------- | ------------------------------------------------- | ----------------------- |
| **Abonnement** | `POST /subscription/checkout` → ouvrir URL        | Checkout LS             |
|                | `GET /subscription/status`                        | Vérifier après paiement |
|                | `POST /subscription/cancel`                       | Annuler                 |
|                | `POST /subscription/resume`                       | Réactiver               |
|                | `GET /subscription/portal`                        | Portail client LS       |
| **Billet**     | `GET /events/{id}/ticket-price`                   | Vérifier prix & dispo   |
|                | `POST /events/{id}/tickets/checkout` → ouvrir URL | Checkout LS             |
|                | `GET /me/tickets`                                 | Vérifier après paiement |
|                | `DELETE /me/tickets/{id}`                         | Rembourser (si assuré)  |
|                | `POST /tickets/{qrCode}/scan`                     | Scanner à l'entrée      |
