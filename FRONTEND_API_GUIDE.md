...
> **Auth** : `Authorization: Bearer {token}` (Sanctum) — requis sur toutes les routes marquées 🔒
...
Valeurs acceptées (non exhaustif, chaîne libre) : `instagram`, `soundcloud`, `spotify`, `youtube`, `tiktok`, `twitter`, `facebook`, `website`
...
```json
{
  ...
  "media": [ /* tableau d'objets Media */ ],
  "social_links": [ /* tableau d'objets SocialLink */ ],
  "stats": {
    "votes_received": 42,
    "events_count": 3,
    "friends_count": 15
...
> Tous les champs sont optionnels. `city` / `postal_code` = saisie manuelle. `latitude` / `longitude` = coordonnées GPS.
...
```json
[
  { "id": 1, "user_id": 1, "platform": "instagram", "url": "https://instagram.com/johndoe" }
]
```
...
### `PUT /me/social-links` 🔒
Met à jour tous les liens sociaux (remplace tout).

**Request :**
```json
[
  { "platform": "instagram", "url": "https://instagram.com/johndoe" },
  { "platform": "soundcloud", "url": "https://soundcloud.com/johndoe" }
]
```

**Response 200 :** tableau de SocialLink mis à jour.
...
[
  {
    ...
    "voting_time_remaining": 3628800,
    "distance_km": 2.4,
    "artists": [ /* tableau d'objets User (ARTIST) */ ],
...
### `GET /artists/{userId}/detail` (public)
...
```json
{
  ...
  "media": [ /* tableau Media */ ],
  "social_links": [ /* tableau SocialLink */ ]
}
```
...
```json
{
  ...
  "media": [
    { "id": 5, "url": "https://...", "path": "media/tracks/...", "position": 0 },
    { "id": 6, "url": "https://...", "path": "media/tracks/...", "position": 1 }
  ]
}
```
...
### `GET /artists/{user}/social-links` (public)
...
[
  ...
  { "id": 2, "platform": "soundcloud", "url": "https://soundcloud.com/johndoe" }
]
```
